...
class WebProducts extends \yii\db\ActiveRecord
{
	...
	public function rules()
	{
		return [
			[['name_ru'], 'required'],
			[['catalog_id','old_image','old_pdf','filename','body_ru','name_uk','translit',
                            'about_ru','about_uk','link',
	...
	public function beforeSave($insert) {
		...
		if($image = UploadedFile::getInstance($this,'image')){			
            ...
            $this->image .=  '.' . $image->extension;
                        $image->saveAs(Yii::getAlias('@frontend/web/upload/web-products/'.$this->image));
			
			$resizeObj = new resize(Yii::getAlias('@frontend/web/upload/web-products/'.$this->image));
			$resizeObj -> resizeImage(270, 250, 'landscape');
                        $resizeObj -> saveImage(Yii::getAlias('@frontend/web/upload/web-products/ico/'.$this->image), 100);
			$resizeObj -> resizeImage(900, 900, 'landscape');
                        $resizeObj -> saveImage(Yii::getAlias('@frontend/web/upload/web-products/big/'.$this->image), 100);
        ...
        if($pdf = UploadedFile::getInstance($this,'pdf')){
            ...
            $pdf->saveAs(Yii::getAlias('@frontend/web/upload/web-products/pdf/'.$this->pdf));
    ...
    public function deletePdf($file){
        if(!empty($file)){
            @unlink(Yii::getAlias('@frontend/web/upload/web-products/pdf/'.$file));
        }
    }

    public function deleteImage($file){
                        if(!empty($file)){
                            @unlink(Yii::getAlias('@frontend/web/upload/web-products/'.$file));
                            @unlink(Yii::getAlias('@frontend/web/upload/web-products/ico/'.$file));
                            @unlink(Yii::getAlias('@frontend/web/upload/web-products/big/'.$file));
                        }            
        }          
