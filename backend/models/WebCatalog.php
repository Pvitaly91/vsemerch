	...
	public function beforeSave($insert) {
		...
		if($image = UploadedFile::getInstance($this,'image')){			
			...
			$this->image = time() . '_' . rand(1, 1000) . '.' . $image->extension;
                        $image->saveAs(Yii::getAlias('@frontend/web/upload/web-catalog/'.$this->image));
			
			$resizeObj = new resize(Yii::getAlias('@frontend/web/upload/web-catalog/'.$this->image));
			$resizeObj -> resizeImage(500, 500, 'crop');
                        $resizeObj -> saveImage(Yii::getAlias('@frontend/web/upload/web-catalog/big/'.$this->image), 100);
        ...
        public function deleteImage($file){ 
                        if(!empty($file)){
                            @unlink(Yii::getAlias('@frontend/web/upload/web-catalog/'.$file));
                            @unlink(Yii::getAlias('@frontend/web/upload/web-catalog/ico/'.$file));
                        }            
        }          
