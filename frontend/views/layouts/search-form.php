...
<script>
    $(document).ready(function(){
        $("#searchInput").keyup(function(){
            ...
            if(str.length >=3){
                $.get(
                    "<?= str_replace("/shop/shop/", "/shop/", Url::to(['shop/catalog/search-ajax'])) ?>?search_str="+str,
                    function(data){
                        $("#searchRes").show();
                        $("#searchRes").html(data);
                    }
        ...
        $(".header__search-svg").click(function(){
          ...
          window.location = "<?= Url::to(['/shop/catalog/search']) ?>?search_str="+$("#searchInput").val();
        })
    });
</script>
<div class="header__search search_box">

    <input class="header__search__input" id="searchInput" placeholder="<?= Yii::t('app', 'Search by products') ?><?/*Пошук по товарах*/?>" />
